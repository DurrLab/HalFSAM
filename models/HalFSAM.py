...
class HalFSAM(nn.Module):
    """ Definition of the HalFSAM network. """
    def __init__(
        self,
        trunk,
        neck,
    ...
    def forward(self, video, track_in_reverse=False):
        if video.dim() == 5:
            B, T, C, H, W = video.size()
            video = video.view(B*T, C, H, W)
        video = video.unsqueeze(1)
        T, _, C, H, W = video.size()
        mems = dict()
        edges = []
        frame_ids = np.arange(0, T)
        if track_in_reverse:
...
def build_HalFSAM(config_name, ckpt_path=None):
...
def build_DexiNed(config_name=None, ckpt_path=None):
    from models.backbones.DexiNed import DexiNed as DexiNed_ori
    model = DexiNed_ori()
    model.requires_grad_(True)
    num_leanrnables = count_learnable_parameters(model)
    print(f'Learnable weights: {num_leanrnables}')
