...
def bdcn_loss2(inputs, targets, l_weight=1.1):
    ...
    mask[mask > 0.] = 1.0 * num_negative / (num_positive + num_negative + 1e-5) #0.1
    mask[mask <= 0.] = 1.1 * num_positive / (num_positive + num_negative + 1e-5)  # before mask[mask <= 0.1]
    # mask[mask == 2] = 0
    inputs= torch.sigmoid(inputs.float()).clamp(1e-6, 1-1e-6)

    assert torch.all((targets >= 0) & (targets <= 1)), "Targets outside [0,1]!"
    assert torch.all((inputs >= 0) & (inputs <= 1)), "Inputs outside [0,1]!"

    cost = torch.nn.BCELoss(mask.float(), reduction='none')(inputs, targets.float())
...
def re_Dice_Loss(inputs, targets, cuda=True, balance=1.1):
    n, c, h, w = inputs.size()
    smooth=1
